...
class RNPeerInterfaceTests: XCTestCase {
    ...
    // Test operability of startup and shutdown
    func testPeerInterfaceStartupAndShutdown() {
        ...
        do {
            ...
            let maxPeers = peer.getMaximumNumberOfPeers()
            XCTAssert(maxPeers == 1, "Incorrect amount of maximum peers: \(maxPeers)")
            
            let sturtupStatus = peer.isActive()
            XCTAssert(sturtupStatus, "Network thread is not running")
            
            peer.shutdownWithDuration(0)
    ...
    func testPeerInterfaceStartupAndShutdownMultipleSockets() {
        ...
        do {
            let socketDescriptors = [RNSocketDescriptor(port: 19132, andAddress: nil), RNSocketDescriptor(port: 19133, andAddress: nil)]
            try peer.startupWithMaxConnectionsAllowed(2, socketDescriptors: socketDescriptors)
    ...
    func testSettingAndGettingMaximumIncomingConnections() {
        ...
        let returnedValue = peer.getMaximumIncomingConnections()
    ...
    func testConvertingGUIDToAddress() {
        ...
        let myGUID = peer.getMyGUID()
        XCTAssertTrue(myGUID > 0, "Returned incorrect GUID")
        
        let address = RNSystemAddress()
        
        let returnedGUID = peer.getGuidFromSystemAddress(address)
    ...
    func testSetOfflinePingData() {
        ...
        let returnedValue = String(data: returnedData, encoding: NSUTF8StringEncoding)
    ...
    func testUnconnectedPing() {
        ...
        let data = pongValue.dataUsingEncoding(NSUTF8StringEncoding)
